...
/*  Version 103 :
  ...
  // return 0 if failure, else return 1  F
*/
...
/* Version 104 :
  G_resize_window 
  was eliminated.
  This seemed unused and the underlying X subroutine 
  behaved strangely with some X servers ... if the requested
  window was larger than the display, the back buffer was 
  changed in size on some X servers...not what was intended.
...
int Init_X (double Dswidth, double Dsheight)
{
    ...
    //    XxDisplay = XOpenDisplay("MAGIC"); // FOR REPL
               // repl no longer needs or wants the MAGIC
    ...
    if (sheight > maxheight) {
      ...
      //      printf("Xx display window width truncated\n") ;
      printf("Xx display window height truncated\n") ;      
      printf("BUT the back buffer IS as specified.\n") ;
